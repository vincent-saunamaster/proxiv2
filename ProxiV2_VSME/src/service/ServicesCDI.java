...
public class ServicesCDI implements IConseiller, IGerant {
	...
	public void effectuerVirement(int montant, Compte c1, Compte c2)
		...
		if (montant < 0) // Test si le montant entr� est inf�rieur � 0
		{
			...
			//pas le même compte !
			if (c1.getIdCompte() == c2.getIdCompte()) {
			} else {
				String type;
				try {
					type = idao.recuperationTypeCompte(c1);

					if (type.equals("epargne")) // Test si le compte est un
												// compte
												// Epargne
					{
						if (montant < c1.getSolde()) // Test si le montant est
														// inferieur au solde du
														// compte
						{
						...
						if (type.equals("courant")) // Test si le compte est un
													// compte Courant
						{
							if ((c1.getSolde() - montant) > -1000) // Test si le
																	// solde du
																	// compte
																	// vir�
																	// est au
	...
	public void ajouterClient(int idcon, Client c) throws LeConseillerADeja10Clients {
		...
		if (idao.compterNombreClient(idcon) < 10) {
			...
			if (c.getTypeClient().equals("particulier") || c.getTypeClient().equals("entreprise")) { // Test
																										// si
																										// client
																										// entreprise
																										// ou
	...
	public void ajouterConseiller(int idge, Conseiller co) {
		...
		/*
		 * Collection<Conseiller> col = g.getConseillers(); // R�cup�re la liste
		 * des conseillers du g�rant col.add(co); //Ajoute le Conseiller co � la
		 * liste col g.setConseillers(col); // Associe la nouvelle liste des
		 * conseillers au g�rant co.setGerant(g); // Associe le nouveau
		 * conseiller � son g�rant
		 */
	...
	public void modifierConseiller(Conseiller c, Adresse a, String telephone) {
		...
		/*
		 * c.setSonAdresse(a); c.setTelephone(telephone);
		 */
	...
	public void supprimerConseiller(Conseiller c, int idge) {
		...
		/*
		 * Collection<Conseiller> col1 = g.getConseillers(); //R�cup�re la liste
		 * des conseillers du g�rant col1.remove(c); //supprimer le conseiller c
	...
	public String effectuerAudit(Agence agence) {
		 ...
		 * Iterator<Conseiller> itCons =
		 * agence.getGerant().getConseillers().iterator(); while
		 * (itCons.hasNext()) { Iterator<Client> itClient =
		 * itCons.next().getClients().iterator(); while (itClient.hasNext()) {
		 * Iterator<Compte> itCompte = itClient.next().getComptes().iterator();
		 * while (itCompte.hasNext()) { Compte eval = itCompte.next(); if
		 * (eval.getClient() instanceof ClientParticulier) { if (eval.getSolde()
		 * < 5000) { res += "" + eval.getClient().getConseiller() +
		 * eval.getClient() + eval + eval.getSolde(); } if (eval.getClient()
		 * instanceof ClientEntreprise) { if (eval.getSolde() < 50000) { res +=
		 * "" + eval.getClient().getConseiller() + eval.getClient() + eval +
		 * eval.getSolde(); }
		 * 
		 * } } } } } return res;
		 */
	...
	private Compte creationCompte(Compte c) {
		 ...
		 * 
		 * CompteEpargne ce = new CompteEpargne(); return ce; }
		 * if(c.getTypeCompte().equals("courant")) { CompteCourant cc = new
		 * CompteCourant(); return cc; }
		 * 
		 * return null;
		 */
	}
	...
	public void ajouterCompteClient(Client c, Compte co)
		...
		/*
		 * IConseiller cs = new Services(); co=cs.creationCompte(co);
		 * 
		 * if(co instanceof CompteEpargne){ //Test si le compte � ajouter est un
		 * Compte Epargne boolean b=true; for (Compte compte : c.getComptes()) {
		 * if(compte instanceof CompteEpargne) { b=false; } } if(b=true){ //
		 * Test pour savoir si le client a d�j� un compte epargne
		 * 
		 * c.setComptes((Collection<Compte>) co);
		 * System.out.println("Le compte Epargne a �t� ajout�."); }else{ throw
		 * new
		 * CompteEpargneExistantException("Le client a d�j� un Compte Epargne."
		 * ); }
		 * 
		 * } else{ if(co instanceof CompteCourant){ // Dans le cas d'un ajout de
		 * Compte Courant boolean b2=true; for (Compte compte : c.getComptes())
		 * { if(compte instanceof CompteCourant) { b2=false; } } if(b2=true){ //
		 * Test pour savoir si le client a d�j� un compte courant
		 * 
		 * c.setComptes((Collection<Compte>) co);
		 * System.out.println("Le compte courant a �t� ajout�.");
		 * 
		 * } else{ throw new
		 * CompteCourantExistantException("Le client a d�j� un Compte Courant."
		 * ); }
		 * 
		 * }
		 * 
	...
	public void supprimerClient(Client c, int idcon) {
		...
		/*
		 * Collection<Client> col = co.getClients(); //R�cup�ration de la liste
		 * des clients du conseiller dans la collection col col.remove(c);
		 * //Suppression du client de la collection co.setClients(col);
		 * //Association de la Collection mise � jour au conseiller
		 */
	}
	...
	public void supprimerCompteClient(Compte co, Client c)
		...
		/*
		 * if(co instanceof CompteEpargne){ //Test si le compte � supprimer est
		 * un Compte Epargne boolean b=false; for (Compte compte :
		 * c.getComptes()) { if(compte == co) { b=true; } } // Test pour savoir
		 * si le client a bien ce compte if(b==true) { co = null;
		 * System.out.println("Le compte Epargne a �t� supprim�.");
		 * 
		 * }else{ throw new
		 * AbsenceDeCompteEpargneException("Le compte n'appartient pas au Client"
		 * ); }
		 * 
		 * } else{ if(co instanceof CompteCourant){ boolean b=false; for (Compte
		 * compte : c.getComptes()) { if(compte == co) { b=true; } }// Dans le
		 * cas d'une suppression du Compte Courant if(b==true){ // Test pour
		 * savoir si le client a d�j� un compte courant
		 * 
		 * 
		 * co=null; System.out.println("Le compte courant a �t� supprim�.");
		 * 
		 * } else{ throw new
		 * AbsenceDeCompteCourantException("Le Client n'a pas ce compte courant."
		 * ); } }
		 * 
		 * }
		 */
	...
	/**
	 * Realisation d'une simulation de cr�dit avec un montant, un taux et une
	 * dur�e de remboursement
	 * 
	...
	public double effectuerSimulationCredit(double montant, int taux, int duree) throws MontantNegatifException {
		...
		/*
		 * double montantARembourserParMois; if (montant<=0) //Test si le
		 * montant entr� est inf�rieur � 0 { montantARembourserParMois = 0;
		 * throw new
		 * MontantNegatifException("montant de remboursement negatif"); } else {
		 * double montantARembourser = montant*(1+(taux/100)); //Calcul de la
		 * somme totale � rembours� (montant avec les interets)
		 * montantARembourserParMois = montantARembourser/duree; //Calcul des
		 * mensualit�s � rembourser par le client } return
	...
	public Placement creerPlacement(String typePlacement) {
		...
		/*
		 * // calcul de la fortune client Collection<Compte> col =
		 * patrimoine.getClient().getComptes(); double fortune = 0;
		 * 
		 * Iterator<Compte> it = col.iterator(); while (it.hasNext()) { fortune
		 * = fortune + (it.next().getSolde()); } // condition de fortune pour
		 * creer un placement if (fortune > 500000) {
		 * 
		 * Collection<Placement> pl = new ArrayList<Placement>(); pl =
		 * patrimoine.getPlacements(); // creer le placement en choisissant la
		 * bourse if(typePlacement.equals("Paris")) {
		 * 
		 * Placement p = new Placement(patrimoine); pl.add(p);
		 * patrimoine.setPlacements(pl); return p; }
		 * 
		 * if(typePlacement.equals("Tokyo")) { Placement p = new
		 * Placement(patrimoine); pl.add(p); patrimoine.setPlacements(pl);
		 * return p; } if(typePlacement.equals("NewYork")) { Placement p = new
		 * Placement(patrimoine); pl.add(p); patrimoine.setPlacements(pl);
		 * return p; }
		 * 
		 * 
	...
	public void supprimerPlacement(Placement placement) {
		...
		/*
		 * 
		 * Collection<Placement> col =
		 * placement.getPatrimoine().getPlacements(); Collection<Placement> col2
		 * = new ArrayList<Placement>(); Iterator<Placement> it =
		 * col.iterator(); while (it.hasNext()) if (it.next().getIdPlacement()
		 * == placement.getIdPlacement()) { // ne fais rien quand on arrive sur
		 * le placement a supprimer } else { col2.add(it.next()); // ajoute les
		 * placements que l'on ne // supprimme pas dans une Collection }
		 * 
	...
	public void activationCarteVisa(Compte c, CarteBancaire cv) {
		...
		/*
		 * c.setCarteBancaire(cv); System.out.println("La carte " + cv
		 * +" a �t� activ�e pour le compte " + c);
		 */
	}
	...
	public void desactivationCarteVisa(Compte c, CarteBancaire cv) {
		...
		/*
		 * c.setCarteBancaire(null); System.out.println("La carte " + cv
		 * +" a �t� supprim�e de compte " + c);
		 */
	}
